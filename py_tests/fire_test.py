...
fire_interval = config.getfloat(section, "fire_interval", fallback=1.6)
...
# 初始化命令结构 (VisionToGimbal 格式)
MODE_CONTROL_NO_FIRE = 1  # 模式1: 控制云台但不开火
MODE_CONTROL_FIRE = 2  # 模式2: 控制云台且开火
mode = MODE_CONTROL_NO_FIRE

# 命令数据初始化：yaw/pitch角度、角速度、角加速度均为0（保持云台位置不变）
yaw = pitch = 0.0
yaw_vel = pitch_vel = 0.0
yaw_acc = pitch_acc = 0.0
...
try:
    while True:
        # 每 fire_interval 秒切换一次开火模式
        if time.time() - last_fire_time > fire_interval:
            mode = MODE_CONTROL_FIRE  # 触发开火
            last_fire_time = time.time()
            print("[DEBUG] Fire command sent!")
        else:
            mode = MODE_CONTROL_NO_FIRE
